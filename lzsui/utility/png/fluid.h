 ...
 * Remark: Must free the return data (free(ret)) when not used!
 */
char *fluid_decode(const char *data, int size, int *width, int *height);

/*
 * convert rgba to argb to make it save in BITMAP 
 */
void rgba2argb(char * rgbaData, int height, int width);
